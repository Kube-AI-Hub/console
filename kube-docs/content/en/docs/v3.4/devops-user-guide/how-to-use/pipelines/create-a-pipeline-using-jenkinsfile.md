...
keywords: 'Kube AI Hub, Kubernetes, Docker, Spring Boot, Jenkins, DevOps, CI/CD, Pipeline'
...
Two types of pipelines can be created in Kube AI Hub: Pipelines created based on a Jenkinsfile in SCM, which is introduced in this tutorial, and [pipelines created through the graphical editing panel](../create-a-pipeline-using-graphical-editing-panel/). The Jenkinsfile in SCM requires an internal Jenkinsfile in Source Control Management (SCM). In other words, the Jenkfinsfile serves as part of SCM. The Kube AI Hub DevOps system automatically builds a CI/CD Jenkins pipeline based on the existing Jenkinsfile of the code repository. You can define workflows such as `stage` and `step`.
...
3. Click the edit icon on the right to edit environment variables.
   ...
   | :--- | :--- | :--- |
   | DOCKER\_CREDENTIAL\_ID | dockerhub-id | The **Name** you set in Kube AI Hub for your Docker Hub account. |
   | GITHUB\_CREDENTIAL\_ID | github-id | The **Name** you set in Kube AI Hub for your GitHub account. It is used to push tags to your GitHub repository. |
   | KUBECONFIG\_CREDENTIAL\_ID | demo-kubeconfig | The **Name** you set in Kube AI Hub for your kubeconfig. It is used to access a running Kubernetes cluster. |
   ...
   | SONAR\_CREDENTIAL\_ID | sonar-token | The **Name** you set in Kube AI Hub for the SonarQube token. It is used for code quality test. |
...
1. Log out of Kube AI Hub and log back in as `project-regular`. Go to the DevOps project `demo-devops` and click **Create**.
...
5. In **Advanced Settings**, select the checkbox next to **Delete outdated branches**. In this tutorial, you can use the default value of **Branch Retention Period (days)** and **Maximum Branches**.
   ...
   **Branch Retention Period (days)** and **Maximum Branches** apply to branches at the same time. As long as a branch meets the condition of either field, it is deleted. For example, if you specify 2 as the retention period and 3 as the maximum number of branches, any branch that exceed either number is deleted. Kube AI Hub prepopulates these two fields with 7 and 5 by default respectively.

   {{</ notice >}} 

6. In **Strategy Settings**, Kube AI Hub offers four strategies by default. You can delete **Discover PRs from Forks** as this strategy will not be used in this example. You do not need to change the setting and can use the default value directly.
   ...
   **Discover PRs from Origin**
   ...
   - **Create two pipelines respectively**. Kube AI Hub creates two pipelines, one based on the source code after the PR is merged into the target branch, and the other based on the source code of the PR itself.
...
1. After a pipeline is created, click its name to go to its details page.
   ...
   - The pipeline details page shows **Sync Status**. It reflects the synchronization result between Kube AI Hub and Jenkins, and you can see the **Successful** icon if the synchronization is successful.  
...
4. The pipeline pauses at the stage `deploy to dev`. You need to click **Proceed** manually. Note that the pipeline will be reviewed three times as `deploy to dev`, `push with tag`, and `deploy to production` are defined in the Jenkinsfile respectively.
   ...
   In Kube AI Hub 3.4, if you do not specify a reviewer, the user that can run a pipeline will be able to continue or terminate the pipeline. If you specify a reviewer, the user who created the pipeline and the user specified will be able to continue or terminate the pipeline.
...
1. To access the Service, log in to Kube AI Hub as `admin` to use the **kubectl** from **Toolbox**. Go to the project `kubesphere-sample-dev`, and click `ks-sample-dev` in **Services** under **Application Workloads**. Obtain the endpoint displayed on the details page to access the Service.
