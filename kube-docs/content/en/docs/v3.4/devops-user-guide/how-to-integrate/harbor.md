...
This tutorial demonstrates how to integrate Harbor into Kube AI Hub pipelines.
...
It is highly recommended that you install Harbor through [the App Store of Kube AI Hub](../../../application-store/built-in-apps/harbor-app/). Alternatively, install Harbor manually through Helm3.
...
1. Log in to Kube AI Hub as `project-regular`, go to your DevOps project and create credentials for Harbor in **Credentials** under **DevOps Project Settings**.
...
2. Copy and paste the following contents into the Jenkinsfile. Note that you must replace the values of `REGISTRY`, `HARBOR_NAMESPACE`, `APP_NAME`, and `HARBOR_CREDENTIAL` with your own values.
   ...
   pipeline {  
     ...
     environment {
       ...
       // ‘robot-test’ is the credential ID you created on the Kube AI Hub console
...
Save the Jenkinsfile and Kube AI Hub automatically creates all stages and steps on the graphical editing panel. Click **Run** to run the pipeline. If everything goes well, the image is pushed to your Harbor registry by Jenkins.
