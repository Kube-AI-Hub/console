...
keywords: 'Kubernetes, docker, DevOps, Jenkins, Go, Kube AI Hub'
description: 'Learn how to build and deploy a Go project using a Kube AI Hub pipeline.'
...
You need to create credentials in Kube AI Hub for the access token created so that the pipeline can interact with Docker Hub for imaging pushing. Besides, you also create kubeconfig credentials for the access to the Kubernetes cluster.

1. Log in to the web console of Kube AI Hub as `project-regular`. In your DevOps project, go to **Credentials** under **DevOps Project Settings** and then click **Create** on the **Credentials** page.
...
3. Click **Create** again and select **kubeconfig** for **Type**. Note that Kube AI Hub automatically populates the **Content** field, which is the kubeconfig of the current user account. Set a **Name** and click **OK**.
...
2. Copy and paste all the content below to the displayed dialog box as an example Jenkinsfile for your pipeline. You must replace the value of `DOCKERHUB_USERNAME`, `DOCKERHUB_CREDENTIAL`, `KUBECONFIG_CREDENTIAL_ID`, and `PROJECT_NAME` with yours. When you finish, click **OK**.
  ...
  pipeline {
     ...
     environment {
       ...
       // 'dockerhubid' is the credentials ID you created in Kube AI Hub with Docker Hub Access Token
       DOCKERHUB_CREDENTIAL = credentials('dockerhubid')
       // the kubeconfig credentials ID you created in Kube AI Hub
       KUBECONFIG_CREDENTIAL_ID = 'go'
       // the name of the project you created in Kube AI Hub, not the DevOps project name
