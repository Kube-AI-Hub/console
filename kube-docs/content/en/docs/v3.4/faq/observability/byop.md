...
keywords: "Monitoring, Prometheus, node-exporter, kube-state-metrics, Kube AI Hub, Kubernetes"
description: "Use your own Prometheus stack setup in Kube AI Hub."
linkTitle: "Bring Your Own Prometheus"
Weight: 16330
---

Kube AI Hub comes with several pre-installed customized monitoring components, including Prometheus Operator, Prometheus, Alertmanager, Grafana (Optional), various ServiceMonitors, node-exporter, and kube-state-metrics. These components might already exist before you install Kube AI Hub. It is possible to use your own Prometheus stack setup in Kube AI Hub v3.4.

## Bring Your Own Prometheus

### Step 1. Uninstall the customized Prometheus stack of Kube AI Hub
...
Kube AI Hub 3.4 was certified to work well with the following Prometheus stack components:
...
Make sure you install **node-exporter** and **kube-state-metrics** if only **Prometheus Operator** and **Prometheus** are installed. **node-exporter** and **kube-state-metrics** are required for Kube AI Hub to work properly.

**If you've already had the entire Prometheus stack up and running, you can skip this step.**

{{</ notice >}}

The Prometheus stack can be installed in many ways. The following steps show how to install it into the namespace `monitoring` using [Prometheus stack manifests in ks-installer](https://github.com/kubesphere/ks-installer/tree/release-3.4/roles/ks-monitor/files/prometheus) (generated from a Kube AI Hub custom version of [kube-prometheus](https://github.com/prometheus-operator/kube-prometheus.git)).

1. Obtain `ks-installer` that Kube AI Hub v3.4 uses.
...
3. Remove unnecessary components. For example, if Grafana is not enabled in Kube AI Hub, you can run the following command to delete the Grafana section in `kustomization.yaml`.
...
### Step 3. Install Kube AI Hub customized stuff to your Prometheus stack
...
Kube AI Hub 3.4 uses Prometheus Operator to manage Prometheus/Alertmanager config and lifecycle, ServiceMonitor (to manage scrape config), and PrometheusRule (to manage Prometheus recording/alert rules).
...
2. Create `kustomization.yaml`, fill the following content.
   ...
   - If you have enabled the alerting component for Kube AI Hub, supplement yaml paths of `thanos-ruler` into `kustomization.yaml`.

   {{</ notice >}}

3. Install the required components of Kube AI Hub.
...
5. Set the Prometheus rule evaluation interval to 1m to be consistent with the Kube AI Hub v3.4 customized ServiceMonitor. The Rule evaluation interval should be greater than or equal to the scrape interval.
...
### Step 4. Change Kube AI Hub's `monitoring endpoint`

Now that your own Prometheus stack is up and running, you can change Kube AI Hub's monitoring endpoint to use your own Prometheus.
...
4. If you have enabled the alerting component of Kube AI Hub, navigate to `prometheusEndpoint` and `thanosRulerEndpoint` of `alerting`, and change the values according to the following sample. Kube AI Hub APIServer will restart automatically to make your configurations take effect.
...
If you enable/disable Kube AI Hub pluggable components following [this guide](../../../pluggable-components/overview/) , the `monitoring endpoint` will be reset to the original value. In this case, you need to change it to the new one.
