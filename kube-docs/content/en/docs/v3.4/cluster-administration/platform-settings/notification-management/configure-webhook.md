...
keywords: 'Kube AI Hub, Kubernetes, custom, platform, webhook'
...
1. Log in to the Kube AI Hub web console as the `platform-admin` user.
...
4. On the **Webhook** tab page, set the following parameters:
   ...
   {{< notice note>}}Currently, Kube AI Hub does not suppot TLS connections (HTTPS). You need to select **Skip TLS verification (insecure)** if you use an HTTPS URL.
