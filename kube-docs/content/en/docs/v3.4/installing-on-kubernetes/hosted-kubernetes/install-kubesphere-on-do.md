---
title: "Deploy Kube AI Hub on DigitalOcean Kubernetes"
keywords: "Kubernetes, Kube AI Hub, DigitalOcean, Installation"
description: "Learn how to deploy Kube AI Hub on DigitalOcean."

weight: 4230
---

This guide walks you through the steps of deploying Kube AI Hub on [DigitalOcean Kubernetes](https://www.digitalocean.com/products/kubernetes/).

## Prepare a DOKS Cluster

A Kubernetes cluster in DO is a prerequisite for installing Kube AI Hub. Go to your [DO account](https://cloud.digitalocean.com/) and refer to the image below to create a cluster from the navigation menu.
...
5. A name for the cluster (for example, _kubesphere-3_)
...
- Wait for DO to assign an external endpoint to `ks-console`, and then access the Kube AI Hub web console with that address.

  {{< notice tip >}}

  Instead of changing the service type to `LoadBalancer`, you can also access Kube AI Hub console via `NodeIP:NodePort` (service type set to `NodePort`). You need to get the public IP of one of your nodes.

  {{</ notice >}}

- Log in to the console with the default account and password (`admin/P@88w0rd`). After login, review the cluster overview and system component status first.
