...
A container runtime must be installed in advance if you want to deploy Kube AI Hub in an offline environment.
...
- Recommended Kubernetes versions for Kube AI Hub 3.4: v1.20.x, v1.21.x, * v1.22.x, * v1.23.x, * v1.24.x, * v1.25.x, and * v1.26.x. For Kubernetes versions with an asterisk, some features of edge nodes may be unavailable due to incompatability. Therefore, if you want to use edge nodes, you are advised to install Kubernetes v1.23. If you do not specify a Kubernetes version, KubeKey will install Kubernetes v1.23.10 by default. For more information about supported Kubernetes versions, see [Support Matrix](../kubekey/#support-matrix).
...
- You can specify a Kube AI Hub version that you want to install (for example, `--with-kubesphere v3.4.1`).
...
- Before you install Kube AI Hub, you can use the information provided under `hosts` (for example, IP addresses and passwords) to test the network connection between the taskbox and other instances using SSH.
...
After the sign-in page is available, log in and confirm that the cluster overview and system components can be loaded normally.
