...
Kube AI Hub project network isolation lets project administrators enforce which network traffic is allowed using different rules. This tutorial demonstrates how to enable network isolation among projects and set rules to control network traffic.
...
For the implementation of the Network Policy, you can refer to [Kube AI Hub NetworkPolicy](https://github.com/kubesphere/community/blob/master/sig-network/concepts-and-designs/kubesphere-network-policy.md).

{{</ notice >}}

## Enable/Disable Project Network Isolation

1. Log in to Kube AI Hub as `project-admin`. Go to your project and select **Network Isolation** in **Project Settings**. By default, project network isolation is disabled.
...
If the default policy does not meet your needs when network isolation is enabled, you can customize your network policy to meet your needs. Currently, you can add custom network policies in Kube AI Hub for traffic within the cluster or incoming traffic outside the cluster.
...
Q: Why cannot the custom monitoring system of Kube AI Hub get data after I enabled network isolation?

A: After you enable custom monitoring, the Kube AI Hub monitoring system will access the metrics of the Pod. You need to allow ingress traffic for the Kube AI Hub monitoring system. Otherwise, it cannot access Pod metrics.

Kube AI Hub provides a configuration item `allowedIngressNamespaces` to simplify similar configurations, which allows all projects listed in the configuration.
