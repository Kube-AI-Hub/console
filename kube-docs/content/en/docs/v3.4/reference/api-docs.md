---
title: "Kube AI Hub API"
keywords: 'Kubernetes, Kube AI Hub, API'
description: 'The REST API is the fundamental fabric of Kube AI Hub. This guide shows you how to access the Kube AI Hub API server.'
linkTitle: "Kube AI Hub API"
weight: 17200
---
...
Kube AI Hub v3.0 moves the functionalities of **ks-apigateway** and **ks-account** into **ks-apiserver** to make the architecture more compact and clear. In order to use the Kube AI Hub API, you need to expose **ks-apiserver** to your client.


### Step 1: Expose the Kube AI Hub API service

If you are going to access Kube AI Hub inside the cluster, you can skip the following section and just use the Kube AI Hub API server endpoint **`http://ks-apiserver.kubesphere-system.svc`**.

On the other hand, you need to expose the Kube AI Hub API server endpoint outside the cluster first.
...
If the identity is correct, the server will respond as shown in the following output. `access_token` is the token to access the Kube AI Hub API Server.
...
If you have everything you need to access the Kube AI Hub API server, make the call using the access token acquired above as shown in the following example to get the node list:
...
The Kube AI Hub API swagger JSON file can be found in the repository https://github.com/kubesphere/kubesphere/tree/release-3.4/api.

- Kube AI Hub specified the API [swagger json](https://github.com/kubesphere/kubesphere/blob/release-3.1/api/ks-openapi-spec/swagger.json) file. It contains all the APIs that are only applied to Kube AI Hub.
- Kube AI Hub specified the CRD [swagger json](https://github.com/kubesphere/kubesphere/blob/release-3.1/api/openapi-spec/swagger.json) file. It contains all the generated CRDs API documentation. It is same as Kubernetes API objects.

You can explore the Kube AI Hub API document from [here](https://kubesphere.io/api/kubesphere) as well.
