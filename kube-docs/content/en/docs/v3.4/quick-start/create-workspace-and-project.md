...
Kube AI Hub needs to be installed in your machine.
...
2. Click **Platform** in the upper-left corner, and then select **Access Control**. In the left nevigation pane, select **Platform Roles**. The built-in roles are shown in the following table.
   ...
   {{< notice note >}}
   Built-in roles are created automatically by Kube AI Hub and cannot be edited or deleted.
   {{</ notice >}}
...
In this step, you create a project using user `project-admin` created in the previous step. A project in Kube AI Hub is the same as a namespace in Kubernetes, which provides virtual isolation for resources. For more information, see [Namespaces](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/).

1. Log in to Kube AI Hub as `project-admin`. In **Projects**, click **Create**.
...
After you finish the above steps, you know that users can be granted different roles at different levels. The roles used in previous steps are all built-in ones created by Kube AI Hub. In this step, you will learn how to define a customized role to meet the needs in your work.

1. Log in to the Kube AI Hub web console as `admin` again and go to **Access Control**. 
...
To create a DevOps project, you must install the Kube AI Hub DevOps system in advance, which is a pluggable component providing CI/CD pipelines, Binary-to-image, Source-to-image, and more. For more information about how to enable DevOps, see [Kube AI Hub DevOps System](../../pluggable-components/devops/).
...
You are now familiar with the multi-tenant management system of Kube AI Hub. In other tutorials, user `project-regular` will also be used to demonstrate how to create applications and resources in a project or DevOps project.
