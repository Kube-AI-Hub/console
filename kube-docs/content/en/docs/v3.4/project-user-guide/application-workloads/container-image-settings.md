---
title: "Pod Settings"
keywords: 'Kube AI Hub, Kubernetes, image, workload, setting, container'
...
When you create Deployments, StatefulSets or DaemonSets, you need to specify a Pod. At the same time, Kube AI Hub provides users with various options to customize workload configurations, such as health check probes, environment variables and start commands. This page illustrates detailed explanations of different properties in **Pod Settings**.
...
The container name is automatically created by Kube AI Hub, which is indicated by `.spec.containers.name`.
...
The CPU resource is measured in CPU units, or **Core** in Kube AI Hub. The memory resource is measured in bytes, or **MiB** in Kube AI Hub.
...
You can select different deployment modes to switch between inter-pod affinity and inter-pod anti-affinity. In Kubernetes, inter-pod affinity is specified as field `podAffinity` of field `affinity` while inter-pod anti-affinity is specified as field `podAntiAffinity` of field `affinity`. In Kube AI Hub, both `podAffinity` and `podAntiAffinity` are set to `preferredDuringSchedulingIgnoredDuringExecution`. You can enable **Edit YAML** in the upper-right corner to see field details.
