...
keywords: 'Kubernetes, Kube AI Hub, multicluster, agent-connection'
description: 'Learn how to import a cluster through agent connection and how the overview page guides the agent.yaml workflow.'
titleLink: "Agent Connection"
weight: 5220
---

The component [Tower](https://github.com/kubesphere/tower) of Kube AI Hub is used for agent connection. Tower is a tool for network connection between clusters through the agent. If the host cluster cannot access the member cluster directly, you can expose the proxy service address of the host cluster. This enables the member cluster to connect to the host cluster through the agent. This method is applicable when the member cluster is in a private environment (for example, IDC) and the host cluster is able to expose the proxy service. The agent connection is also applicable when your clusters are distributed across different cloud providers.

To use the multi-cluster feature using an agent, you must have at least two clusters serving as the host cluster and the member cluster respectively. A cluster can be defined as the host cluster or the member cluster either before or after you install Kube AI Hub. For more information about installing Kube AI Hub, refer to [Installing on Linux](../../../installing-on-linux/) and [Installing on Kubernetes](../../../installing-on-kubernetes/).

![](/images/docs/v3.x/cluster-overview/agent-connection-flow.svg)

## Agent Guidance on the Overview Page

After you create a member cluster in the console and choose **Agent connection**, the cluster **Overview** page shows a three-step guide while the cluster is waiting to join:

1. Log in to the member cluster over SSH and create an `agent.yaml` file.
2. Copy the generated agent manifest from the console into `agent.yaml`.
3. Run `kubectl create -f agent.yaml` on the member cluster and wait for the agent to report status.

{{< notice info >}}

The `agent.yaml` content displayed on the overview page is generated automatically by the host cluster and can be copied directly to the member cluster.
...
{{< tab "Kube AI Hub has been installed" >}}

If you already have a standalone Kube AI Hub cluster installed, you can set the value of  `clusterRole` to `host` by editing the cluster configuration.
...
{{< tab "Kube AI Hub has not been installed" >}}

You can define a host cluster before you install Kube AI Hub either on Linux or on an existing Kubernetes cluster. If you want to [install Kube AI Hub on Linux](../../../installing-on-linux/introduction/multioverview/#1-create-an-example-configuration-file), you use a `config-sample.yaml` file. If you want to [install Kube AI Hub on an existing Kubernetes cluster](../../../installing-on-kubernetes/introduction/overview/#deploy-kubesphere), you use two YAML files, one of which is `cluster-configuration.yaml`.

To set a host cluster, change the value of `clusterRole` to `host` in `config-sample.yaml` or `cluster-configuration.yaml` accordingly before you install Kube AI Hub.
...
If you install Kube AI Hub on a single-node cluster ([All-in-One](../../../quick-start/all-in-one-on-linux/)), you do not need to create a `config-sample.yaml` file. In this case, you can set a host cluster after Kube AI Hub is installed.
...
If a LoadBalancer plugin is available for the cluster, you can see a corresponding address for `EXTERNAL-IP` of tower, which will be acquired by Kube AI Hub. In this case, the proxy service is set automatically. That means you can skip the step to set the proxy. Execute the following command to verify if you have a LoadBalancer.
...
If you already have a standalone Kube AI Hub cluster installed, you can set the value of `clusterRole` to `member` by editing the cluster configuration.
...
If you install Kube AI Hub on a single-node cluster ([All-in-One](../../../quick-start/all-in-one-on-linux/)), you do not need to create a `config-sample.yaml` file. In this case, you can set a member cluster after Kube AI Hub is installed.
...
1. Log in to the Kube AI Hub console as `admin` and click **Add Cluster** on the **Cluster Management** page.
...
4. Create an `agent.yaml` file on the member cluster, then copy and paste the generated agent deployment into the file. Run the following command on the member cluster:

   ```bash
   kubectl create -f agent.yaml
   ```

5. Make sure the proxy address is reachable from the member cluster. Once the agent is up and running, the overview page switches from the waiting state to the normal cluster dashboard.