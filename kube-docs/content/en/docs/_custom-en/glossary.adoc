...
|Workspace
|A workspace is the basic logical unit for managing projects and members, serving as the foundation of the Kube AI Hub multi-tenant system.

|Workspace Member
|A user invited to work in a workspace with specific permissions.

|Project
|A project in Kube AI Hub corresponds to a namespace in Kubernetes.
...
|KubeKey
|A new installation tool that offers flexible installation options, allowing users to install Kubernetes alone or Kubernetes and Kube AI Hub together. KubeKey also supports various installation methods, such as All-in-One, multi-node, and offline installation. Users only need to prepare the configuration file and execute the relevant commands.

// |ks-installer
// |An installation package for deploying Kube AI Hub on an existing Kubernetes cluster.
...
|Edge Node
|Nodes deployed in edge environments and managed by Kube AI Hub, typically used to process local data and provide low-latency services.
...
|Ingress
|Ingress is used to aggregate services within the cluster and provide external access. Each Ingress contains mapping rules from domain names and their subpaths to different services. In Kube AI Hub, Ingress corresponds to Kubernetes Ingress.
|===
