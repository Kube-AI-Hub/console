...
* **Protocol**: The protocol that the application in the target container actually listens on. To use the application governance features provided by Kube AI Hub, ensure that the protocol selected here matches the one the application in the target container actually listens on.

* **Name**: The name of the service port. The application governance features of Kube AI Hub require the service port name to have the protocol name in lowercase as a prefix. To use the application governance features provided by Kube AI Hub, use the lowercase protocol name as the prefix for the port name (e.g., **http-**).
