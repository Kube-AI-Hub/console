---
title: "开发环境（Notebook 实例）介绍"
keywords: "行业大模型平台, Notebook, JupyterLab, VS Code, Eclipse Theia, 交互式开发"
description: "通过开发环境功能，用户可一键创建独立的计算环境，直接使用平台算力资源进行数据分析、模型训练和实验。"
linkTitle: "开发环境介绍"
weight: 6100
---
...
开发环境页面统一承载 Notebook 类实例的创建与管理能力。用户可直接使用平台算力资源进行数据分析、模型训练和实验，无需自行配置环境，即开即用，高效便捷，支持快速迭代和交互式开发。
...
- **持久化存储**：开发实例工作目录支持持久化，关闭实例后数据不丢失。
