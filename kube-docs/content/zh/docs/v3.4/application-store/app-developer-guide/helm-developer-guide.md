---
title: "Helm 开发者指南"
keywords: 'Kubernetes, Kube AI Hub, Helm, 开发'
description: '开发基于 Helm 的应用。'
linkTitle: "Helm 开发者指南"
weight: 14410
---

您可以上传应用的 Helm Chart 至 Kube AI Hub，以便具有必要权限的租户能够进行部署。本教程以 NGINX 为示例演示如何准备 Helm Chart。

## 安装 Helm

如果您已经安装 Kube AI Hub，那么您的环境中已部署 Helm。如果未安装，请先参考 [Helm 文档](https://helm.sh/docs/intro/install/)安装 Helm。
...
当您向 Kubernetes 部署基于 Helm 的应用时，可以直接在 Kube AI Hub 控制台上编辑 `values.yaml` 文件。
...
要在 Kube AI Hub 中使用 HTTP 或 HTTPS URL 添加仓库，您需要事先向对象存储上传一个 `index.yaml` 文件。在 `nginx` 的上一个目录中使用 Helm 执行以下命令，创建索引文件。
...
- 有关何如向 Kube AI Hub 添加仓库的更多信息，请参见[导入 Helm 仓库](../../../workspace-administration/app-repository/import-helm-repository/)。
...
现在您已经准备好了基于 Helm 的应用，您可以将它上传至 Kube AI Hub 并在平台上进行测试。
