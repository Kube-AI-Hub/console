---
title: "在 Kube AI Hub 中部署 etcd"
keywords: 'Kubernetes, Kube AI Hub, etcd, 应用商店'
description: '了解如何从 Kube AI Hub 应用商店中部署 etcd 并访问服务。'
linkTitle: "在 Kube AI Hub 中部署 etcd"
weight: 14210
---

[etcd](https://etcd.io/) 是一个采用 Go 语言编写的分布式键值存储库，用来存储供分布式系统或机器集群访问的数据。在 Kubernetes 中，etcd 是服务发现的后端，存储集群状态和配置。

本教程演示如何从 Kube AI Hub 应用商店部署 etcd。
...
应用部署后，您可以在 Kube AI Hub 控制台上使用 etcdctl 命令行工具与 etcd 服务器进行交互，直接访问 etcd。
...
4. Kube AI Hub 集群内的客户端可以通过 `<app name>.<project name>.svc.<K8s domain>:2379`（例如本教程中是 `etcd-rscvf6.demo-project.svc.cluster.local:2379`） 访问 etcd 服务。
