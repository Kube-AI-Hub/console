---
title: "在 Kube AI Hub 中部署 ClickHouse 集群"
keywords: 'Kube AI Hub, Kubernetes, 安装, ClickHouse'
description: '了解如何从 Kube AI Hub 应用商店部署 ClickHouse。'
...
本教程演示了如何在 Kube AI Hub 上部署 ClickHouse Operator 和 ClickHouse 集群。
...
1. 以 `admin` 身份登录 Kube AI Hub 的 Web 控制台，并使用**工具箱**中的 **Kubectl** 执行以下命令来安装 ClickHouse Operator。建议至少准备 2 个可用集群节点。
...
3. 将仓库成功导入到 Kube AI Hub 之后，在列表中可查看 ClickHouse 仓库。


### 步骤 3：部署 ClickHouse 集群

1. 以 `project-regular` 身份登录 Kube AI Hub 的 Web 控制台。在 `demo-project` 项目中，进入**应用负载**下的**应用**页面，点击**创建**。
...
1. 以 `project-regular` 身份登录 Kube AI Hub 的 Web 控制台。
...
1. 以 `admin` 身份登录 Kube AI Hub 的 Web 控制台，将鼠标悬停在右下角的锤子图标上，选择 **Kubectl**。
