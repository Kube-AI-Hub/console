---
title: "离线安装"
keywords: 'Kubernetes, Kube AI Hub, 离线, 安装'
description: '了解在离线环境中安装 Kube AI Hub 的最佳实践。'

linkTitle: "离线安装"
weight: 4310
---

离线安装几乎与在线安装相同，不同之处是您必须创建一个本地仓库来托管 Docker 镜像。本教程演示了如何在离线环境中将 Kube AI Hub 安装到 Kubernetes 上。
...
与在现有 Kubernetes 集群上在线安装 Kube AI Hub 相似，您也需要事先下载 `cluster-configuration.yaml` 和 `kubesphere-installer.yaml`。
...
2. 编辑 `cluster-configuration.yaml` 添加您的私有镜像仓库。例如，本教程中的仓库地址是 `dockerhub.kubekey.local`，将它用作 `.spec.local_registry` 的值，如下所示：
   ...
   您可以在该 YAML 文件中启用可插拔组件，体验 Kube AI Hub 的更多功能。有关详情，请参考[启用可插拔组件](../../../pluggable-components/)。
...
现在，您可以通过 `http://{IP}:30880` 使用默认帐户和密码 `admin/P@88w0rd` 访问 Kube AI Hub 的 Web 控制台。
