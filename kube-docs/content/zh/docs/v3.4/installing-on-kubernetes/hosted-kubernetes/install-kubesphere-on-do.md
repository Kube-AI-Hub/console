---
title: "在 DigitalOcean 上部署 Kube AI Hub"
keywords: 'Kubernetes, Kube AI Hub, DigitalOcean, 安装'
description: '了解如何在 DigitalOcean 上部署 Kube AI Hub。'
...
在 DO 上创建一个标准的 Kubernetes 集群是安装 Kube AI Hub 的前提条件。登录您的 [DO account](https://cloud.digitalocean.com/) 帐户，然后在导航菜单中，参考下图创建集群。
...
## 在 DOKS 上安装 Kube AI Hub

现在集群已准备就绪，您可以按照以下步骤安装 Kube AI Hub：
...
现在已经安装了 Kube AI Hub，可以按照以下步骤访问 Kube AI Hub 的 Web 控制台。
...
- 等待 DO 为 `ks-console` 服务分配外部端点，然后使用该地址访问 Kube AI Hub 的 Web 控制台。
 ...
 除了将服务类型更改为`LoadBalancer`，您还可以通过`NodeIP:NodePort`（服务类型设置为`NodePort`）访问 Kube AI Hub 控制台，这种方式需要获取任意节点的公共 IP。
