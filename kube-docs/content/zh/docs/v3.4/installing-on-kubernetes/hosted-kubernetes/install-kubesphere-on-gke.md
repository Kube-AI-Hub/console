---
title: "在 Google GKE 上部署 Kube AI Hub"
keywords: 'Kubernetes, Kube AI Hub, GKE, 安装'
description: '了解如何在 Google Kubernetes Engine 上部署 Kube AI Hub。'
...
- 在 GKE 上创建一个标准的 Kubernetes 集群是安装 Kube AI Hub 的前提条件，转到导航菜单然后参考下图创建集群。
...
除了将服务类型更改为`LoadBalancer`，还可以通过`NodeIP:NodePort`（服务类型设置为 `NodePort`）访问 Kube AI Hub 控制台，注意需要在防火墙规则中打开 30880 端口。
...
上面的示例演示了默认的最小安装过程，要在Kube AI Hub中启用其他组件，请参阅[启用可插拔组件](../../../pluggable-components/)。
