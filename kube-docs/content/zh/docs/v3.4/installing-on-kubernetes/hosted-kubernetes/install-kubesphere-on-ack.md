...
本指南将介绍如何在[阿里云容器服务 ACK](https://www.aliyun.com/product/kubernetes/) 上部署并使用 Kube AI Hub 3.4 平台。
...
由于阿里云 Prometheus node-exporter 端口与 Kube AI Hub 冲突，这里不勾选 Prometheus 监控服务， 不安装阿里云 Prometheus 组件。
...
- 使用 ACK 生成的 external-ip 访问 Kube AI Hub 的 Web 控制台, 默认帐户和密码（`admin/P@88w0rd`）。
...
由于阿里云 ACK 已经在 kube-system 命名空间部署 Metrics-server，请勿开启 Kube AI Hub metrics-server 插件，否则部署失败。
