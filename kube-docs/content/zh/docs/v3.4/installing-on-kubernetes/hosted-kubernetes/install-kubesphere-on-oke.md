---
title: "在 Oracle OKE 上部署 Kube AI Hub"
keywords: 'Kubernetes, Kube AI Hub, OKE, 安装, Oracle-cloud'
description: '了解如何在 Oracle Cloud Infrastructure Container Engine 上部署 Kube AI Hub。'

weight: 4260
---

本文演示在 [Oracle Kubernetes Engine](https://www.oracle.com/cn/cloud/compute/container-engine-kubernetes.html) 上部署 Kube AI Hub 的步骤。

## 创建 Kubernetes 集群

1. 在 OKE 上创建一个标准的 Kubernetes 集群是安装 Kube AI Hub 的前提条件。在导航栏中，请参考下图创建集群。
...
5. 在 Cloud Shell 中，粘贴该命令以便之后可以执行 Kube AI Hub 安装命令。
...
Kube AI Hub 安装完成后，您可以通过 `NodePort` 或 `LoadBalancer` 的模式访问 Kube AI Hub 的 Web 控制台。

1. 通过以下命令查看 Kube AI Hub 控制台的服务状态。

    ```bash
    kubectl get svc -n kubesphere-system
    ```

2. 输出中若 `ks-console` 的 `TYPE` 为 `NodePort`，则可以继续将其改为 `LoadBalancer`，以便暴露外部 IP 地址。
...
4. 将 `type` 字段对应的值修改为 `LoadBalancer`，然后保存配置。

5. 再次执行以下命令，确认 `EXTERNAL-IP` 字段已经分配到可访问地址。
...
上面的示例演示了默认的最小安装过程，要在 Kube AI Hub 中启用其他组件，请参阅[启用可插拔组件](../../../pluggable-components/)。
