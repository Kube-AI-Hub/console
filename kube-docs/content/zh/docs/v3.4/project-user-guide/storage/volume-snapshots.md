...
description: '了解如何在 Kube AI Hub 中管理持久卷声明的快照。'
linkTitle: "卷快照"
weight: 10330
---
...
1. 以 `project-regular` 用户登录 Kube AI Hub Web 控制台并进入项目。在左侧导航栏选择**存储**下的**卷快照**。
...
1. 以 `project-regular` 用户登录 Kube AI Hub Web 控制台并进入项目。在左侧导航栏选择**存储**下的**持久卷声明**，在页面右侧选择需要创建快照的持久卷声明。
...
1. 以 `project-regular` 用户登录 Kube AI Hub Web 控制台，进入快照详情页面，然后点击**创建卷**来使用快照。其他步骤与直接创建持久卷声明基本相同。
...
1. 以 `project-regular` 用户登录 Kube AI Hub Web 控制台并进入项目。在左侧导航栏选择**存储**下的**持久卷声明**，然后点击**创建**。
