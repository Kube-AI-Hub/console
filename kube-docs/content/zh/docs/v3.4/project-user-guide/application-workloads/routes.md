---
title: "应用路由"
keywords: "Kube AI Hub, Kubernetes, 路由, 应用路由"
description: "了解应用路由（即 Ingress）的基本概念以及如何在 Kube AI Hub 中创建应用路由。"
weight: 10270
---

本文档介绍了如何在 Kube AI Hub 上创建、使用和编辑应用路由。

Kube AI Hub 上的应用路由和 Kubernetes 上的 [Ingress](https://kubernetes.io/docs/concepts/services-networking/ingress/#what-is-ingress) 相同，您可以使用应用路由和单个 IP 地址来聚合和暴露多个服务。
...
1. 以 `project-admin` 身份登录 Kube AI Hub 的 Web 控制台，然后访问您的项目。
...
1. 登出 Kube AI Hub 的 Web 控制台，以  `project-regular` 身份登录，并访问同一个项目。
...
2. 选择一种模式来配置路由规则，点击 **√**，然后点击**下一步**。
   ...
   * **自动生成**：Kube AI Hub 自动以`<服务名称>.<项目名称>.<网关地址>.nip.io` 格式生成域名，该域名由 [nip.io](https://nip.io/) 自动解析为网关地址。该模式仅支持 HTTP。
