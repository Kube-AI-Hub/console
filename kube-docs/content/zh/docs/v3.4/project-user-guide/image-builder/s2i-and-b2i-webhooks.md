...
Kube AI Hub 提供 Source-to-Image (S2I) 和 Binary-to-Image (B2I) 功能，以自动化镜像构建、推送和应用程序部署。在 Kube AI Hub 3.4 中，您可以配置 S2I 和 B2I Webhook，以便当代码仓库中存在任何相关活动时，自动触发镜像构建器。
...
1. 以 `admin` 身份登录 Kube AI Hub Web 控制台。在左上角点击**平台管理**，然后选择**集群管理**。
...
1. 登出 Kube AI Hub 并以 `project-regular` 用户登回。然后转到 `demo-project`。
