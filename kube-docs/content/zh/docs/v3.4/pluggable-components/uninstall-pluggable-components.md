...
description: "学习如何在 Kube AI Hub上卸载所有可插拔组件。"
linkTitle: "卸载可插拔组件"
Weight: 6940
---

[启用 Kube AI Hub 可插拔组件之后](../../pluggable-components/)，还可以根据以下步骤卸载他们。请在卸载这些组件之前，备份所有重要数据。
## 准备工作
...
- 使用 `admin` 身份登录 Kube AI Hub Web 控制台，左上角点击**平台管理**，选择**集群管理**，在**定制资源定义**中搜索 `ClusterConfiguration`。有关更多信息，请参见[启用可插拔组件](../../pluggable-components/)。
...
2. 运行以下命令：
   ...
   Kube AI Hub 3.4 通知系统为默认安装，您无需卸载。
...
对于 NetworkPolicy 组件，禁用它不需要卸载组件，因为其控制器位于 `ks-controller-manager` 中。如果想要将其从 Kube AI Hub 控制台中移除，将 CRD `ClusterConfiguration`  配置文件中参数 `ks-installer` 中 `network.networkpolicy.enabled` 的值从 `true` 改为 `false`。
