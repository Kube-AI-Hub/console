...
GlusterFS 和 Heketi 搭建好之后，就可以在客户端机器上安装 GlusterFS，并使用 KubeKey 创建一个存储类型为 GlusterFS 的 Kube AI Hub 集群。
...
5. 创建 JSON 文件以配置 Heketi。
   ...
   在安装 GlusterFS 作为 Kube AI Hub 集群的存储类型时，必须提供帐户 `admin` 及其 `Secret` 值。
