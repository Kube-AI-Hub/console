---
title: "配置企业微信通知"
keywords: 'Kube AI Hub, Kubernetes, 企业微信, 通知, 告警'
...
### 步骤 3：在 Kube AI Hub 控制台配置企业微信通知

您必须在 Kube AI Hub 控制台提供企业微信的相关 ID 和凭证，以便 Kube AI Hub 将通知发送至您的企业微信。

1. 使用具有 `platform-admin` 角色的用户（例如，`admin`）登录 Kube AI Hub Web 控制台。
...
配置企业微信通知并添加 ID 后，您需要启用 [Kube AI Hub 告警系统](../../../../pluggable-components/alerting/)，并为[工作负载](../../../../project-user-guide/alerting/alerting-policy/)或[节点](../../../cluster-wide-alerting-and-notification/alerting-policy/)创建告警策略。告警触发后，接收设置中添加的用户或部门将收到通知消息。
...
- 如果您更新了企业微信服务器配置，Kube AI Hub 将根据最新配置发送通知。

- 默认情况下，Kube AI Hub 大约每 12 小时针对同一告警发送通知。告警重复间隔主要由 `kubesphere-monitoring-system` 项目中 `alertmanager-main` 密钥的 `repeat_interval` 所控制。您可以按需自定义重复间隔。

- Kube AI Hub 设有内置告警策略，在不设置任何自定义告警策略的情况下，只要内置告警策略被触发，您的企业微信仍能接收通知消息。
