---
title: "配置 Slack 通知"
keywords: 'Kube AI Hub, Kubernetes, Slack, 通知'
...
## 在 Kube AI Hub 控制台上配置 Slack 通知

您必须在 Kube AI Hub 控制台提供 Slack 令牌用于认证，以便 Kube AI Hub 将通知发送至您的频道。
...
配置 Slack 通知并添加频道后，您需要启用 [Kube AI Hub 告警](../../../../pluggable-components/alerting/)，并为工作负载或节点创建告警策略。告警触发后，列表中的全部频道都将能接收通知。

{{< notice note >}}

- 如果您更新了 Slack 通知配置，Kube AI Hub 将根据最新配置发送通知。

- 默认情况下，Kube AI Hub 大约每 12 小时针对同一告警发送通知。告警重复间隔期主要由 `kubesphere-monitoring-system` 项目中 `alertmanager-main` 密钥的 `repeat_interval` 所控制。您可以按需自定义间隔期。

- Kube AI Hub 拥有内置告警策略，在不设置任何自定义告警策略的情况下，只要内置告警策略被触发，您的 Slack 频道仍能接收通知。
