---
title: "配置 Webhook 通知"
keywords: 'Kube AI Hub, Kubernetes, 自定义, 平台, Webhook'
...
1. 以 `platform-admin` 用户身份登录 Kube AI Hub Web 控制台。
...
4. 在 **Webhook** 标签页，设置如下参数：
   ...
   {{< notice note>}}目前，Kube AI Hub 不支持 TLS 连接（HTTPS）。如果您使用 HTTPS URL，则需要选择**跳过 TLS 认证（不安全）**。
