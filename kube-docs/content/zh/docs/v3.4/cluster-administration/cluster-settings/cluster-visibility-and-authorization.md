...
title: "集群可见性和授权"
keywords: "集群可见性, 集群管理"
description: "了解如何设置集群可见性和授权，以及如何从集群概览页进入相关设置。"
linkTitle: "集群可见性和授权"
weight: 8610
---

在 Kube AI Hub 中，您可以通过授权将一个集群分配给多个企业空间，让企业空间资源都可以在该集群上运行。同时，一个企业空间也可以关联多个集群。拥有必要权限的企业空间用户可以使用分配给该企业空间的集群来创建多集群项目。
...
如果您已经在使用集群概览页，可以直接通过**基本信息**卡片中的**集群可见性**字段进入本页面。这一入口仅在启用多集群功能时显示。

## 设置集群可见性

### 在创建企业空间时选择可用集群

1. 使用拥有创建企业空间权限的用户登录 Kube AI Hub，例如 `ws-manager`。
...
1. 使用拥有集群管理权限的帐户登录 Kube AI Hub，例如 `admin`。

2. 点击左上角的**平台管理**，选择**集群管理**。从列表中选择一个集群查看集群信息。您也可以先打开该集群的**概览**页，再通过**基本信息**卡片中的**集群可见性**字段进入当前设置页。
