...
Kube AI Hub 在 `v3.4.0` 版本集成了 OpenSearch 的 `v1` 和 `v2` 版本，并作为 `logging`、`events` 和 `auditing` 组件的默认后端存储。
...
在 Kube AI Hub `v3.4.0` 版本中，默认 OpenSearch 为 `logging`、`events` 或 `auditing` 组件的后端存储。 配置如下：
...
```
Kube AI Hub 版本低于 `v3.4.0`的，请先[升级](https://github.com/kubesphere/ks-installer/tree/release-3.4#upgrade)。
...
3. 在**自定义资源**中，点击 `ks-installer` 右侧的操作图标，选择**编辑 YAML**。
...
如果您使用的是 Kube AI Hub 内部的 OpenSearch，并且想把它改成您的外部 OpenSearch，请按照以下步骤操作。
...
1. 所有用户都可以使用日志查询功能。使用任意帐户登录控制台，点击右下角的工具箱图标，在弹出菜单中选择**日志查询**。

2. 在弹出窗口中，您可以看到日志数量的时间直方图、集群选择下拉列表以及日志查询栏。

3. 您可以点击搜索栏并输入搜索条件，可以按照消息、企业空间、项目、资源类型、资源名称、原因、类别或时间范围搜索日志（例如，输入时间范围：最近 10 分钟，来搜索最近 10 分钟的日志）。或者，点击时间直方图中的柱状图，Kube AI Hub 会使用该柱状图的时间范围进行日志查询。
