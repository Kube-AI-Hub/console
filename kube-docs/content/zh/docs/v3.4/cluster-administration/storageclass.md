...
description: "了解 PV、PVC 和存储类的基本概念，并演示如何在 Kube AI Hub 中管理存储类和 PVC。"
linkTitle: "存储类"
weight: 8800
---

本教程演示集群管理员如何管理 Kube AI Hub 中的存储类。
...
Kube AI Hub 支持基于存储类的[动态卷供应](https://kubernetes.io/zh/docs/concepts/storage/dynamic-provisioning/)，以创建 PV。
...
6. 在**存储系统**页签，选择一个存储系统，点击**下一步**。
   ...
   在 Kube AI Hub 中，您可以直接为 `QingCloud-CSI`、`GlusterFS` 和 `Ceph RBD` 创建存储类。您也可以为其他存储系统创建自定义存储类。
...
QingCloud CSI 是 Kubernetes 上的 CSI 插件，供青云QingCloud 存储服务使用。Kube AI Hub 控制台上可以创建 QingCloud CSI 的存储类。

#### 准备工作

- QingCloud CSI 在青云QingCloud 的公有云和私有云上均可使用。因此，请确保将 Kube AI Hub 安装至二者之一，以便可以使用云存储服务。
- Kube AI Hub 集群上已经安装 QingCloud CSI 插件。有关更多信息，请参见[安装 QingCloud CSI](https://github.com/yunify/qingcloud-csi#installation)。
...
如果 Kube AI Hub 不直接支持您的存储系统，您可以为存储系统创建自定义存储类型。下面的示例向您演示了如何在 Kube AI Hub 控制台上为 NFS 创建存储类。
...
NFS 与部分应用不兼容（例如 Prometheus），可能会导致容器组创建失败。如果确实需要在生产环境中使用 NFS，请确保您了解相关风险或咨询 Kube AI Hub 技术支持 support@kubesphere.cloud。
...
- **设为默认存储类**：将此存储类设为集群的默认存储类。一个 Kube AI Hub 集群中仅允许设置一个默认存储类。
