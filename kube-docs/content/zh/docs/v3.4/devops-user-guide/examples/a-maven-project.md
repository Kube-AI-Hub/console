...
description: '学习如何使用 Kube AI Hub 流水线构建并部署 Maven 项目。'
...
Kube AI Hub DevOps 中有针对 Maven 项目的工作流，如下图所示，它使用 Jenkins 流水线来构建和部署 Maven 项目。所有步骤均在流水线中进行定义。
...
Jenkins Agent 通过节点上的 Docker 存储卷 (Volume) 挂载目录。流水线可以缓存一些特殊目录，例如 `/root/.m2`，这些特殊目录用于 Maven 构建并在 Kube AI Hub DevOps 中用作 Maven 工具的默认缓存目录，以便依赖项包下载和缓存到节点上。
