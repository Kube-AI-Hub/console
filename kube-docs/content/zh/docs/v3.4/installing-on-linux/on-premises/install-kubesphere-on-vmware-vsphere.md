---
title: "在 VMware vSphere 安装 Kube AI Hub"
keywords: 'kubernetes, kubesphere, VMware vSphere, installation'
description: '了解如何在 VMware vSphere 上创建高可用 Kube AI Hub 集群。'

weight: 3510
---

对于生产环境，我们需要考虑集群的高可用性。如果关键组件（例如 kube-apiserver，kube-scheduler 和 kube-controller-manager）都在同一主节点上运行，一旦主节点出现故障，Kubernetes 和 Kube AI Hub 将不可用。因此，我们需要通过用负载均衡器配置多个主节点来设置高可用性集群。您可以使用任何云负载平衡器或任何硬件负载平衡器（例如F5）。另外，Keepalived 和 HAproxy 或 NGINX 也是创建高可用性集群的替代方法。
...
- 参考[多节点安装指南](../../introduction/multioverview/)，确保您已经知道如何安装多节点 Kube AI Hub 集群。本教程重点介绍如何配置负载均衡器实现高可用。
...
如果最后返回`Welcome to Kube AI Hub`，则表示已安装成功。
...
使用上述日志中给定的访问地址进行访问，进入到 Kube AI Hub 的登录界面并使用默认帐户（用户名`admin`，密码`P@88w0rd`）即可登录平台。
