---
title: "Kube AI Hub 在华为云 ECS 高可用实例"
keywords: "Kubesphere 安装， 华为云， ECS， 高可用性， 高可用性，  负载均衡器"
description: "了解如何在华为云虚拟机上创建高可用的 Kube AI Hub 集群。"
...
- 请遵循该[指南](https://github.com/kubesphere/kubekey)，确保您已经知道如何将 Kube AI Hub 与多节点集群一起安装。有关用于安装的 config.yaml 文件的详细信息。本教程重点介绍配置华为云负载均衡器服务高可用安装。
...
spec:
  ...
  openpitrix:          # Whether to install Kube AI Hub App Store. It provides an application store for Helm-based applications, and offer application lifecycle management
    enabled: true
  servicemesh:         # Whether to install Kube AI Hub Service Mesh (Istio-based). It provides fine-grained traffic management, observability and tracing, and offer visualization for traffic topology
    enabled: true
```
...
###  执行命令创建集群
 ...
 # 查看 Kube AI Hub 安装日志  -- 直到出现控制台的访问地址和登录帐户
