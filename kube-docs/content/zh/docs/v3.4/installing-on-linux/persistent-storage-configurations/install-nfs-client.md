---
title: "安装 NFS Client"
keywords: 'Kube AI Hub, Kubernetes, 存储, 安装, 配置, NFS'
description: '使用 KubeKey 搭建 Kube AI Hub 集群并配置 NFS 存储。'
linkTitle: "安装 NFS Client"
weight: 3330
---

本教程演示了如何搭建 Kube AI Hub 集群并配置 NFS 存储。
...
3. 访问稍后想要下载 KubeKey 到其上的一台客户端机器（任务机）。创建一个配置文件，其中包含 NFS 服务器的全部必要参数，KubeKey 将在安装过程中引用该文件。
   ...
   - `storageClass.defaultClass` 字段决定是否将 NFS-client Provisioner 的存储类型设置为默认存储类型。如果您输入 `false`，KubeKey 将安装 [OpenEBS](https://github.com/openebs/openebs) 来提供本地卷，您在集群上创建工作负载时，不会动态供应本地持久卷。安装 Kube AI Hub 之后，您可以直接在控制台上更改默认存储类型。
...
1. 指定您想要安装的 Kubernetes 版本和 Kube AI Hub 版本，例如：
...
4. 保存文件，执行以下命令安装 Kubernetes 和 Kube AI Hub：
...
您可以使用命令行或者从 Kube AI Hub 的 Web 控制台来验证  NFS-client 是否安装成功。
...
3.  选择**存储** > **存储类型**，可以看到集群中可用的存储类型。
   ...
   有关如何在 Kube AI Hub 控制台上创建持久卷声明的更多信息，请参见[持久卷声明](../../../project-user-guide/storage/volumes/)。
