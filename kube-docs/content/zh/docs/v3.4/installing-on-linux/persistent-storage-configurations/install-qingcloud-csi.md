...
keywords: 'Kube AI Hub, Kubernetes, QingCloud CSI, 安装, 配置, 存储'
description: '使用 KubeKey 搭建 Kube AI Hub 集群并配置 QingCloud CSI 存储。'
linkTitle: "安装 QingCloud CSI"
weight: 3320
---

如果您打算在[青云QingCloud](https://www.qingcloud.com/) 上安装 Kube AI Hub，可以选择 [QingCloud CSI](https://github.com/yunify/qingcloud-csi) 作为底层存储插件。

本教程演示了如何使用 KubeKey 搭建 Kube AI Hub 集群及配置 QingCloud CSI 以提供存储服务。
...
您可以使用命令行或者通过 Kube AI Hub 的 Web 控制台来验证 QingCloud CSI 是否安装成功。
...
3. 选择**存储**下的**存储类型**，可以看到集群中可用的存储类型。
   ...
   有关如何在 Kube AI Hub 控制台创建持久卷声明的更多信息，请参见[持久卷声明](../../../project-user-guide/storage/volumes/)。
