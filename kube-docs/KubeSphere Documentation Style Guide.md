# Kube AI Hub Documentation Style Guide

This style guide provides a set of editorial guidelines for those who are writing documentation for Kube AI Hub.
...
- Image format: PNG.
- Make sure images in your guide match the content. For example, you mention that users need to log in to Kube AI Hub using a user of a role; this means the account that displays in your image is expected to be the one you are talking about. It confuses your readers if the content you are describing is not consistent with the image used.
...
Kube AI Hub is a **distributed operating system managing cloud-native applications** with Kubernetes as its kernel.
...
- For values, strings, fields or parameters in yaml files, put them within ``.
  ...
  | Do                                                                                                  | Don't                                                                                           |
  | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
  | Change the value of `auditing.enabled` to `false` to stop receiving auditing logs from Kube AI Hub. | Change the value of auditing.enabled to false to stop receiving auditing logs from Kube AI Hub. |
